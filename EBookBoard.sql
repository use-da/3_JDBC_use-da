...
ALTER TABLE GOODS ADD(SOLD_OUT CHAR(1) DEFAULT 'N' CHECK(SOLD_OUT IN ('N','Y')) );	

UPDATE GOODS SET DEAL_PLACE = '서대문구 연희동' WHERE GOODS_NO =5;
...
COMMENT ON COLUMN SR_BOARD.MEMBER_NO IS '신청자 회원번호';
COMMENT ON COLUMN SR_BOARD.PHONE IS '신청자 전화번호';


CREATE SEQUENCE SEQ_SR_BOARD_NO NOCACHE;

INSERT INTO SR_BOARD 
VALUES(SEQ_SR_BOARD_NO.NEXTVAL, 4);

SELECT*FROM SR_BOARD ;

ALTER TABLE SR_BOARD ADD(DELETE_SB CHAR(1) DEFAULT 'N' CHECK( DELETE_SB IN ('Y','N')));

ALTER TABLE MEMBER ADD(PHONE VARCHAR(20) REFERENCES SR_BOARD);

ALTER TABLE MEMBER DROP(PHONE);

ALTER TABLE MEMBER MODIFY ADDRESS VARCHAR(2000);

UPDATE MEMBER SET ADDRESS='성북구 성북동' WHERE MEMBER_NO=17;

UPDATE SR_BOARD SET DELETE_SB='N' WHERE SR_BOARD_NO=11;


COMMIT;

SELECT SR_BOARD_NO, MEMBER_NO, 
(SELECT SELLER_RIGHT FROM MEMBER M WHERE M.MEMBER_NO = S.MEMBER_NO) SELLER_RIGHT
FROM SR_BOARD S
ORDER BY SR_BOARD_NO DESC;

ALTER TABLE SR_BOARD ADD(PHONE VARCHAR(20) DEFAULT NULL);

DROP TABLE REPORT_BOARD;

DROP SEQUENCE SEQ_SR_BOARD_NO;

----------------------------------------------------------------------
CREATE TABLE REPORT_BOARD(
	RB_NO NUMBER PRIMARY KEY,
	MEMBER_NO NUMBER REFERENCES MEMBER,
	BAD_MEMBER VARCHAR(200) NOT NULL,
	REPORT_CONTENT VARCHAR(2000) DEFAULT NULL
); 

CREATE SEQUENCE SEQ_RB_NO NOCACHE;
