...
public class EmployeeView {
	...
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				input=-1; //반복문 첫바퀴에서 잘못 입력시 종료되는 상황 방지
				sc.nextLine(); //이볅 버퍼에 남아있는 잘못 입력된 문자열 제거해 무한 반복 방지
	...
	private void selectAll() {
		...
		//DB에서 전체 사원 정보를 조회해 List<Employee>형태로 반환하는 dao.selectAll()메서드 호출
	...
	private void selectEmpId() {
		System.out.print("<사번이 일치하는 사원 정보 조회>");
		
		//사번 입력 받기
		int empId=inputEmpId();
		
		//입력 받은 사번을 DAO의 selectEmpId()메서드로 전달하여 조회된 사원 정보 반환 받기
		Employee emp=dao.selectEmpId(empId);  //한 행을 받기 때문에 VO에서 받는다
	...
	private void insertEmployee() {
		System.out.println("<사원 정보 추가>");
	      ...
	      //입력받은 값을 Employee객체에 담아 DAO로 전달
	      ...
	      //INSERT, UPDATE, DELETE 같은 DML구문은 수행 후 테이블에 반영된 행의 개수를 반환함
	      //->조건이 잘못된 경우 반영된 행이 없으므로 0반환
	...
	private void updateEmployee() {
		...
		//기본 생성자로 객체 생성 후 setter를 이용해 초기화 (***실제로 자주 사용하는 방법)
	...
	private void deleteEmployee() {
		...
		char input=sc.next().toUpperCase().charAt(0);   //Y/N 대소문자 구분없이 입력->Upper로 모두 대문자변환
	...
	private void selectDeptEmp() {
		...
		List<Employee>deptEmpList=dao.selectDeptEmp(deptTitle);
		
		printAll(deptEmpList);
	...
	private void selectSalaryEmp() {
		...
		List<Employee>salaryEmpList=dao.selectSalaryEmp(inputsalary);
		
		printAll(salaryEmpList);
