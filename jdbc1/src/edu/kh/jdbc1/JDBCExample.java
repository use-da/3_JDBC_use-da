...
public class JDBCExample {
	public static void main(String[]args) {
		...
		//2. DBMS(Oracle)
		//3. Oracle에서 Java 애플리케이션과 연결할 때 사용할 JDBC를 상속 받아 구현한 클래스 모음(ojdbc11.jar 라이브러리)
		//  ->OracleDriver.class (JDBC 드라이버) 이용
		
		//1단계 : JDBC객체 참조 변수 선언 (java.sql패키지의 인터페이스)
		...
		//java.sql인터페이스 유의
		//Connection을 통해
		//SQL문을 DB에 전달해 실행하고 생성된 결과(ResultSet, 성공한 행의 개수)을 반환하는데 사용되는 개체
		...
		try {
			...
			//1. DB연결에 필요한 Oracle JDBC Driver메모리에 로드하기
			...
			//DriverManager : 메모리에 로드된 JDBC 드라이버를 이용해서 Connection객체를 만드는 역할 
			...
			//oracle.jdbc.driver.T4CConnection@7bedc48a
			
			//3.SQL작성
			//***JAVA에서 작성되는 SQL은 마지막에 ;(세미콜론)을 찍지 않는다
			...
			//5. 생성된 Statement 객체에 sql을 적재하여 실행 후 결과를 반환받아 rs변수에 저장
			rs=stmt.executeQuery(sql);
			//executeQuery() : SELECT문 수행 메서드, ResultSet반환
			
			//3단계 : SQL을 수행해 반환받은 결과(ResultSet)를 한 행씩 접근해 컬럼 값 얻어오기
			while(rs.next()) {
				//rs.next() : rs가 참조하는 ResultSet 객체
				//			  첫번째 컬럼부터 순서대로 한 행씩 이동하며 다음 행이 있을 경우 true를 반환
				//			  없으면 false 반환
				...
				Date hireDate = rs.getDate("HIRE_DATE"); //java.sql.Date import
			...
			//ResultSet,Statement,Connection닫기(생성 역순으로 닫는 것을 권장)
